...
import '../assets/css/catalogpage.css';
        ...
        axios.get(`${serverUrl}api-books/books/`)
            .then(response => {
                ...
                if (filteredBooks.length > 0) {
                    ...
                    setError(`По запросу ${user_request} ничего не найдено`);
                }
                setLoading(false);
            })
            .catch(() => {
    ...
    return (
        <div>
            ...
            <div className="CatalogPage-title">
                <h2 style={{ marginBottom: '10px' }}>{user_request}</h2>
                <p>Вот что у нас есть по этому жанру</p>
            </div>
            { loading ? (
                <p style={{
                    marginTop: '160px'
